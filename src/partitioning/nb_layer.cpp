...
namespace SharedMap {
    ...
    void nb_layer_thread_work_oversubscribed(u64 local_idx,
        ...
        while (local_idx < layer_size) {
            ...
            if (n_assigned_threads > 1) {
                // let new threads work on the other graphs in the layer
            }
    ...
    */
    ...
    void nb_layer_work(u64 local_idx,
                       std::shared_ptr<std::atomic<u64>> global_idx,
        ...
        while (local_idx < layer_size) {
            ...
            if (n_assigned_threads > 1) {
                // check if we can start other threads

                // pull at most n_assigned_threads - 1 indices, that can be processed
                std::vector<u64> idxs;
                for (u64 i = 0; i < n_assigned_threads - 1; i++) {
                    u64 new_idx = global_idx->fetch_add(1);
                    if (new_idx < layer_size) {
                        // valid task in the layer
                        idxs.push_back(new_idx);
                    } else {
                        // no more tasks in the layer
                        break;
                    }
                }
                ...
                if (!idxs.empty()) {
                    // spawn threads
                    u64 n_items                                       = idxs.size() + 1;
                    u64 n_local_assigned_threads_1 = (n_assigned_threads / n_items) + (n_assigned_threads > n_items) * (0 < n_assigned_threads % n_items);
                    for (u64 t = 1; t < n_items; ++t) {
                        u64 n_local_assigned_threads = std::max((u64)1, n_assigned_threads / n_items) + (n_assigned_threads > n_items) * (t < n_assigned_threads % n_items);
                    ...
                    n_assigned_threads = n_local_assigned_threads_1;
                }
            }
    ...
    std::vector<u64> solve_nb_layer(const Graph& original_g,
        ...
        std::shared_ptr<std::atomic<u64>> global_idx = std::make_shared<std::atomic<u64>>(1);
        std::atomic<u64> n_inactive_threads          = 0;
